          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:inline-flex overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-black/5">
              <button
                ...
                className={`p-2 ${view === 'list' ? 'bg-white text-brand-emerald' : 'text-gray-400 hover:text-gray-600'}`}
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
              </button>
              <button
                onClick={() => setView('grid')}
                className={`p-2 ${view === 'grid' ? 'bg-white text-brand-emerald' : 'text-gray-400 hover:text-gray-600'}`}
              >
